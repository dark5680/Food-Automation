...
$(document).ready(function(){
	...
	$(document).on("click","."+deletebutton,function(){
		var id = $(this).attr("id");
		if(id){
			if(confirm("می خواهید این کاربر را حذف کنید؟")==true){
				ajax("rid="+id,"del");
				reservedInfo();
			}
		}
	...
	$(document).on("click","."+editbutton,function(){
		...
		if(id && editing == 0 && tdediting == 0){
			...
			for(i=0;i<columns.length;i++){
				if(i==0){
					var val = $(document).find("."+table+" tr[id="+id+"] td[id='cId']").html();
					html +='<td>'+val+'</td>';
				}
				else{
...
createInput = function(i,str){
	...
	if(inputType[i] == "text"){
		if(i==4 || i==0 || i==1){
			input = '<input onkeypress="return isNumberKey(event);" type='+inputType[i]+' name='+columns[i]+' placeholder="'+placeholder[i]+'" value='+str+' >';
		}
		else
...
ajax = function (params,action){
	$.ajax({
		type: "POST", 
		url: "../resources/library/userTable/ajax.php", 
		  ...
		  switch(action){
			case "save":
				...
				if(response.success == 1){
					...
					html += "<td class='table-color'>"+parseInt(seclastRow-1)+"</td>";
					for(i=0;i<columns.length;i++){
		...
		error: function(){
			alert("خطایی به وجود آمده است.لطفا دوباره امتحان کنید");
		}
	});
}